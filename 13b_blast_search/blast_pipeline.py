...
class BlastPipeline:
    def __init__(self, 
                 ...
                 reference_dir: str = "../13a_new_reference_operon_extraction/output",
        ...
        self.query_files = {
            ...
            'noncoding': self.reference_dir / 'operon_regulatory_nt.fasta'
        }
    ...
    def prepare_blast_db(self):
        ...
        source_fasta = self.query_files['nt']
        db_index = ref_db_dir / 'operon_genes_nt.nsq'
        lock_file = ref_db_dir / 'operon_genes_nt.lock'

        if not source_fasta.exists():
            raise FileNotFoundError(f"Reference nucleotide FASTA not found: {source_fasta}")

        # Simple filesystem lock so only one array task (or rerun) prepares the DB
        lock_acquired = False
        while not lock_acquired:
            try:
                ...
                # If another process is building the DB, wait until it finishes
                if db_index.exists():
                    self.logger.info("Reference database already prepared; reusing existing files.")
                    return
                time.sleep(2)

        try:
            if not db_fasta.exists():
                shutil.copyfile(source_fasta, db_fasta)
...
def main():
    ...
    parser.add_argument('--ref-dir', default='../13a_new_reference_operon_extraction/output',
